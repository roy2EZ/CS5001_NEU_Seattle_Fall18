...
def draw_black(x,y): 
    ...
    othello.penup() 
    othello.hideturtle()
    update_black_lst(x,y)
    
     

# update lists
def update_black_lst(x,y):
    empty_position_set.remove((x,y))
    black_set.add((x,y))
    

def update_white_lst(x,y):
    empty_position_set.remove((x,y))
    white_set.add((x,y))



# get position of x row and y column from mouse click
def click_to_draw():
    s = turtle.getscreen()
